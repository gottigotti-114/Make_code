class Janken
  ...
  def initialize(name,hp,defence,magic1_stock,magic2_stock,magic3_stock)
    @name = name
    @damage, @add_damage = 0, 0
    @magic1, @magic2, @magic3 = "攻撃ダウン", "守備ダウン", "攻撃力アップ"
    @magic1_stock, @magic2_stock, @magic3_stock = magic1_stock,magic2_stock,magic3_stock
    @my_hp = hp
    @defence = defence
  ...
  def output_janken
    srand(Time.now.to_i + rand(100) + $grobal_rand_num += 1)
    ran_num = rand(8) + 1
    case ran_num
    when 1..3
      ans = "グー"
...
#enemyが相手側(bot)
#myが自分側
#引数(HP,守備力,攻撃ダウンストック数,守備ダウンストック数,攻撃力アップストック数)
enemy = Janken.new("悪のしたっぱ",200,100,2,2,2)
my = Janken.new("あなた",200,100,2,2,2)
...
while retia == false
  if turn == 1
    ...
    puts
    srand(Time.now.to_i + rand(100) + $grobal_rand_num += 1)
    r_num1 = rand(2)  #相手側の魔法"攻撃ダウン"の使用数を決める
    srand(Time.now.to_i + rand(100) + $grobal_rand_num += 1)
    r_num2 = rand(2)    #相手側の魔法"守備ダウン"の使用数を決める
    srand(Time.now.to_i + rand(100) + $grobal_rand_num += 1)
    r_num3 = rand(2)#相手側の魔法"攻撃力アップ"の使用数を決める
    if r_num1 != 0
      r_num1.times do
        enemy.magic = "攻撃ダウン"
      end
