...
### 何回ループしても処理が終わらない「無限ループ」に注意が必要。rubyの場合　ctrlキー + cキー　で強制終了

## 使う繰り返し構文について

### whileとtimes等、どちらを使うか迷ったときは繰り返す回数が分かっているかで考える。
...
### 配列に入っている値のことを要素という
### 配列に値を入れると先頭の要素から、0から始まる番号が振られる。この番号のことをインデックスという
...
```
### 変数には、先頭から要素が代入され、全ての要素を取り出したらループが終わる
...
## 文字を使った位置の指定
- ### /a ... 正規表現開始の/のすぐ後ろに書くことによって行の先頭を表す
- ### z/ ... 正規表現終了の/の直前に書くことで行末を表す
- ### Z/ ... 正規表現終了の/の直前に書くことで行末を表す（行末が改行だったら改行の前）

## 正規表現を変数に入れる
### 正規表現も一つのインスタンスとして扱うことができる。
### 以下の書き方で正規表現を変数に入れることができる
```rb
変数 = Regexp.new("パターン") #ここではシングルクォートで囲まないと、エスケープシーケンスが効かなくなる
変数 = /パターン/
...
## 最短マッチ
### 正規表現を使ってマッチする文字列を調べると最長の区間がマッチする。
### 最短の部分をマッチさせるには
```rb
長さ指定?
```
...
## 文字列の置き換え
### 正規表現を使って文字列を置き換えるときはgsubメソッドを使う。
```rb
# マッチした場所を置き換える
文字列.gsub(パターン,置き換える文字)
```
### ただし、gsubは文字列を置き換えるのではなく、置き換えた内容を戻すメソッドになり、元の文字列は変更しない。このように実行したインスタンスの内容を変更しないメソッドを非破壊的メソッドという。
### またgsub!とすると元の文字列を変更する。このようなメソッドを破壊的メソッドという。

### また、パターンマッチした先頭だけ置き換えることもできる。
```rb
文字列.sub(パターン,文字列)
```
### subも破壊的メソッドとなり、破壊的メソッドはsub!となる
# ここまでがテスト範囲